...
* **Plasmid, Virus, and Chromosomal Localization:** While NAPs are predominantly encoded on the bacterial chromosome to regulate core housekeeping functions, plasmid-borne and prophage-encoded (viral) NAPs are frequently associated with facilitating xenogeneic gene acquisition, horizontal gene transfer, and regulatory adaptation.
...
graph TD
    ...
    D -- No --> H[Discard]
    E --> F(geNomad Location Classification)
...
3. **Genomic Location Prediction (geNomad):**
   geNomad is run via Apptainer using a localized container (`genomad_1.12.0--pyhdfd78af_0.sif`) and its database. It predicts whether contigs containing H-NS hits are plasmids, viruses, or chromosomes.
   * **Prophage Overlap Logic:** For integrated proviruses (prophages), geNomad specifies the start and end coordinates of the prophage. An H-NS/StpA hit on a prophage-harboring contig is classified as `Virus` only if it falls within the prophage coordinates; otherwise, it is correctly annotated as `Chromosome`.
...
6. **Parallel Concurrency & Resource Guarding:**
   * **Parallel Scanning and Classification:** Gene prediction and HMMER scanning are distributed across CPU threads using process pools. geNomad classifications are run concurrently, significantly reducing cohort runtime.
   * **Memory Concurrency Guard:** To prevent Out-Of-Memory (OOM) failures under Slurm workloads, the pipeline monitors `SLURM_MEM_PER_NODE` and dynamically caps the concurrent classifier worker count:
...
### 4. Genomic Location
* **`Location`**: The predicted replication origin of the protein-coding gene:
  * **`Chromosome`**: The gene resides on the host chromosome (or fell outside prophage/proviral coordinates on a chromosomal contig).
  * **`Plasmid`**: The gene resides on a plasmid.
  * **`Virus`**: The gene resides inside an integrated prophage/provirus (within its coordinates) or on a viral contig.
  * **`Unknown`**: Fallback value if the classification did not run or failed.
